...
describe("POST /auth/login", function() {
  ...
  // TESTS BUG #6
  test("should not allow an invalid username/password to log in", async function() {
    const response = await request(app)
      .post("/auth/login")
      .send({
        username: "u1",
...
describe("GET /users", function() {
  ...
  // TESTS BUG #1
  test("should return only basic info", async function() {
    ...
    expect(response.body).toEqual(usersObj)
  });
});
...
describe("GET /users/[username]", function() {
  ...
  // TESTS BUG #2
  test("should return a 404 if user not found", async function() {
    const response = await request(app).get("/users/invaliduser").send({ _token: tokens.u1 });;
...
describe("PATCH /users/[username]", function() {
  ...
  test("should patch data if user", async function() {
    const response = await request(app)
      .patch("/users/u1")
      .send({ _token: tokens.u1, first_name: "new-fn1" }); // u1 is user, not admin
    expect(response.statusCode).toBe(200);
    expect(response.body.user).toEqual({
      username: "u1",
      first_name: "new-fn1",
