...
router.patch('/:username', authUser, requireLogin, async function(req, res,next) {
  try {
    ...
    //   if (field === "username" || field === "password" || field === "admin") {
    //     throw new ExpressError("Trying to update invalid field", 401)
    //   }
    // }
        // Fix B
    const validFields = ["first_name", "last_name", "phone", "email"];
    const invalidFields = Object.keys(fields).filter(field => !validFields.includes(field))
